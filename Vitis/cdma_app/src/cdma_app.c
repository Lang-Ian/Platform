...
#define DMA_CTRL_IRPT_INTR	XPAR_FABRIC_I_DGRM_WRAPPER_DGRM_I_AXI_CDMA_0_CDMA_INTROUT_INTR
...
static u32 BUFFER_BYTESIZE	= (XPAR_AXICDMA_0_M_AXI_DATA_WIDTH * XPAR_AXICDMA_0_M_AXI_MAX_BURST_LEN);
...
int main()
{
    ...
    printf("Ian's modification\n");

//goto end;
    SrcPtr = (u32*)SourceAddr;
    printf("Writing 0x%x to address 0x%x.\n", (unsigned) 0x5CAFF01D, (unsigned) SrcPtr );
   *SrcPtr++ = 0x5CAFF01D;
    printf("Writing 0x%x to address 0x%x.\n", (unsigned) 0xDEADBEEF, (unsigned) SrcPtr );
   *SrcPtr++ = 0xDEADBEEF;
   	SrcPtr = (u32*)SourceAddr;
   	poll = *SrcPtr;
    printf("Read 0x%x from address 0x%x.\n", poll, (unsigned) SrcPtr++ );
   	poll = *SrcPtr;
    printf("Read 0x%x from address 0x%x.\n", poll, (unsigned) SrcPtr++ );
